...
class Dynasty:
    ...
    def join(self,validatorAddress,deposit):
        '''
        ...
        if not deposit or deposit <=0:
            logger.warning('warning: validator %s please pay the deposit'%validatorAddress)
            return
        if validatorAddress not in self.join_community:
            self.deposit_bank[validatorAddress]=[deposit,self.current_epoch]
            self.dynasties[self.current_epoch+1][0].add(validatorAddress)
            #join
            self.join_community.append(validatorAddress)
    ...
    def quit(self,validatorAddress):
        ...
        args:
            validatorAddress: the identity of new validator
        '''
        if validatorAddress in self.dynasties[self.current_epoch+1][1]:
            #join_dynasty changed to quit dynasty(d+2+5)
            self.deposit_bank[validatorAddress][1] = self.current_epoch+2+self.withdraw_delay
        ...
        elif validatorAddress in self.dynasties[self.current_epoch+1][0]:
            # join_dynasty changed to quit dynasty(d+2+5)
            self.deposit_bank[validatorAddress][1] = self.current_epoch + 2 + self.withdraw_delay
            self.dynasties[self.current_epoch + 1][2].append(validatorAddress)
            self.dynasties[self.current_epoch + 1][0].remove(validatorAddress)
        else:
            logger.warning('warning: %s not in the new and expert validator', str(validatorAddress))
            # print('wrong')
